...
func (r *RedisHandler) ConnectRedis(ra *resources.RedisAccount) {
	...
	r.Client = redis.NewClient(&redis.Options{
		Addr:         ra.URL + ":" + strconv.Itoa(ra.Port),
...
func (r *RedisHandler) Scan(key string) ([]string, error) {
	...
	if len(res) <= 0 {
		return res, errors.New(resources.NotFound)
	}
	return res, nil
}
...
func (r *RedisHandler) buildPool(ra *resources.RedisAccount) {
	formatClient := fmt.Sprintf("%s:%d", ra.URL, ra.Port)
...
func (r *RedisHandler) buildRedisHandler(ra *resources.RedisAccount, idx []string) {
